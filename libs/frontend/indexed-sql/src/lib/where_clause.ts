...
export function replace_comparision_operand_reference(
  ...
  const where_literals_copy: string[] = JSON.parse(
    JSON.stringify(where_literals)
  );
  ...
  where_literals_copy.forEach((p, i) => {
  ...
  return where_literals_copy.filter((p) => p !== null);
}
...
export function squeeze_comparision_operator_operand(
  ...
  const number_of_comparision_operators_in_where_array: number = found_number_of_comparision_operators(
    where_literals
  );
...
export function evaluate_comparision_statement(
  ...
  for (let index = 0; index < comparision_cell.length; index++) {
    const left_operand = comparision_cell[index].left_operand;
    const right_operand = comparision_cell[index].right_operand;
