...
import { do_data_modification, make_select_literals, parse_data_filter_and_manupulation_part, parse_union_datasource, select_given_columns_from_dataset, single_select } from '../single-table/select_clause';
import { single_table_where_clause } from '../where_clause';
import { find_bracket_close_index, generalTrim, retrive_next_database_version, unite_sources } from '../utils';
...
export function find_alies_column_name(col_literals: string[]) {
  ...
  const find_col_name = (ini_index: number) => {
    if (col_literals[ini_index] === '"') {
      ...
      const corresponding_double_quote_index = col_literals.indexOf('"', ini_index + 1)
      if (corresponding_double_quote_index > -1) {
        const final_column_name = col_literals.slice(ini_index + 1, corresponding_double_quote_index)
    ...
    if (col_literals[ini_index] === '[') {
      ...
      if (corres_closing_bracket_index > -1) {
        const final_column_name = col_literals.slice(ini_index + 1, corres_closing_bracket_index)
        return final_column_name.join(" ").trim()
      } else {
...
export function extract_columns_info_of_multi_select(multi_select_literals: string[]) {
  // anything that is between SELECT and FROM is column info
  // find the first index of SELECT keyword
  const SELECT_keyword_index = multi_select_literals.findIndex((val) => val.toUpperCase() === 'SELECT')
  const FROM_keyword_index = multi_select_literals.findIndex((val) => val.toUpperCase() === 'FROM')
  if (SELECT_keyword_index > -1 && FROM_keyword_index > -1) {
    // slice the columns parts
    const sliced_col_parts = multi_select_literals.slice(SELECT_keyword_index + 1, FROM_keyword_index)
    ...
    throw new Error("We need SELECT and FROM keyword in multi query statement.");
  }
}
...
export function multiple_select_data_source(multi_select_literals: string[]) {
  ...
  for (let index = 0; index < multi_select_literals.length; index++) {
    ...
    if (literal === '(') {
      ...
      const index_of_closing_bracket = find_bracket_close_index(multi_select_literals, 'right', index + 1)
    ...
    if (literal === 'INNER') {
      ...
      if (multi_select_literals[index + 1].toUpperCase().trim() === 'JOIN') {
        ...
        data_sources.push({ type: found_type, data: multi_select_literals.slice(found_index, index) })
        data_sources.push({ type: 'join_type', data: multi_select_literals.slice(index, index + 2).join(" ").trim() })
        found_index = index + 2
        found_type = 'data_source'
        index = index + 1
        continue
      } else {
        throw new Error("missing join keyword , use INNER JOIN instead of INNER");
    ...
    if (literal === 'LEFT') {
      if (multi_select_literals[index + 1].toUpperCase().trim() === 'OUTER' && multi_select_literals[index + 2].toUpperCase().trim() === 'JOIN') {
        data_sources.push({ type: found_type, data: multi_select_literals.slice(found_index, index) })
        data_sources.push({ type: 'join_type', data: multi_select_literals.slice(index, index + 3).join(" ").trim() })
        found_index = index + 3
        found_type = 'data_source'
        index = index + 2
        continue
      } else {
        throw new Error("missing OUTER JOIN keyword , use LEFT OUTER JOIN instead of LEFT");
      }
    }
    ...
    if (index === multi_select_literals.length - 1) {
      // last index
      // slice data
      data_sources.push({ type: found_type, data: multi_select_literals.slice(found_index) })
      found_index = -1
    }
...
export async function is_data_source_union(data_source: string[]) {
  ...
  if (data_source[0].toUpperCase() === '(') {
    const matching_bracket_index = find_bracket_close_index(data_source, 'right', 1)
    if (matching_bracket_index > -1) {
      // slice the part
      // that part actual union
      const sliced_part = data_source.slice(1, matching_bracket_index)
      trimed_form = (await generalTrim(sliced_part.join(" ").trim(), '(')).split(" ")
    } else {
  ...
  for (let index = 0; index < trimed_form.length; index++) {
    ...
    if (literal === '(') {
      // find the closing bracket
      const closing_bracket_index = find_bracket_close_index(trimed_form, 'right', index + 1)
...
// todo - pending test
export async function retrive_single_select_datasource(datasource_literals: string[], database_name_default: string) {
  let select_string: string
  let alies_name: string

  if (datasource_literals[0].toUpperCase() === '(') {
    const closing_bracket_index = find_bracket_close_index(datasource_literals, 'right', 1)
    if (closing_bracket_index > -1) {
      select_string = await generalTrim(datasource_literals.slice(1, closing_bracket_index).join(" "), '(')
      // find the alies
      if (datasource_literals[closing_bracket_index + 1].toUpperCase() === 'AS') {
  ...
  const final_result = await single_select(select_string, database_name_default)
...
export async function retrive_base_source_data(datasource_literals: string[], database_name_default: string) {
  ...
  const multi_database_connection = await openDB(database_name, version_db.current_version)
...
// todo - pending test
export async function retrive_union_datasource(datasource_literals: string[], database_name_default: string) {
  // union datasource
  // extract the data source and alies
  let union_datasource_row: string[]
  let alies_name: string

  for (let index = 0; index < datasource_literals.length; index++) {
    const literal = datasource_literals[index].toUpperCase()
    if (literal === '(') {
      const closing_bracket_index = find_bracket_close_index(datasource_literals, 'right', index + 1)
      if (closing_bracket_index > -1) {
        union_datasource_row = datasource_literals.slice(index + 1, closing_bracket_index)
        // find the alies
        if (datasource_literals[closing_bracket_index + 1].toUpperCase() === 'AS') {
          alies_name = datasource_literals[closing_bracket_index + 2]
        } else {
  ...
  const trimed_value = (await generalTrim(union_datasource_row.join(" "), '(')).split(" ")
  ...
  for (let index = 0; index < sources.length; index++) {
    ...
    const trimed_query = await generalTrim((data.data as string[]).join(" "), '(')
...
//todo - need test
export async function retrive_source_data(data_source: string[], database_name_default: string) {
  ...
  if (await is_data_source_union(data_source)) {
    const union_data = await retrive_union_datasource(data_source, database_name_default)
    return union_data
  } else if (await is_single_select_datasource(data_source)) {
    const single_select_data = await retrive_single_select_datasource(data_source, database_name_default)
    return single_select_data
  } else {
    // base data source
    const base_data = await retrive_base_source_data(data_source, database_name_default)
    return base_data
  }
...
export async function perform_join(left_table_source: any[], right_table_source: any[], join_type: join_type, on_subclause?: string[]) {
  ...
  if (join_type === 'INNER JOIN') {
    ...
    for (let left_index = 0; left_index < left_table_source.length; left_index++) {
      for (let right_index = 0; right_index < right_table_source.length; right_index++) {
        ...
        const result = await single_table_where_clause(intermediate_obj, on_subclause.join(" ").trim())
        if (result) {
    ...
    for (let left_index = 0; left_index < left_table_source.length; left_index++) {
      ...
      if (!is_there_matching_in_right_table) {
        ...
        const final_object = { ...left_table_source[left_index], ...right_table_object_structure }
    ...
    for (let right_index = 0; right_index < right_table_source.length; right_index++) {
      ...
      if (!is_there_matching_in_left_table) {
        ...
        const final_object = { ...right_table_source[right_index], ...left_table_object_structure }
        final_result.push(final_object)
      }
...
// main multi select
export async function multiple_select(select_string: string, database_name_default: string) {
  const m_select_literals = make_select_literals(select_string)
  // extract the multi select datasource
  const multi_select_data_source = multiple_select_data_source(m_select_literals)
  ...
  for (let index = 0; index < multi_select_data_source.length; index++) {
    ...
    if (index === 0) {
      left_table = await retrive_source_data(multi_select_data_source[index].data, database_name_default)

      if (multi_select_data_source[index + 1].type === 'join_type' && multi_select_data_source[index + 2].type === 'data_source') {
        if (multi_select_data_source[index + 3].type === 'on_condition') {

          right_table = await retrive_source_data(multi_select_data_source[index + 2].data, database_name_default)
          intermediate_data = await perform_join(left_table, right_table, multi_select_data_source[index + 1].data, multi_select_data_source[index + 3].data)
          index = 3
          continue
        } else if (multi_select_data_source[index + 1].data === 'INNER JOIN') {

          right_table = await retrive_source_data(multi_select_data_source[index + 2].data, database_name_default)
          intermediate_data = await perform_join(left_table, right_table, multi_select_data_source[index + 1].data)
          index = 2
          continue
        } else {
      ...
      if (multi_select_data_source[index].type === 'join_type' && multi_select_data_source[index + 1].type === 'data_source') {
        if (multi_select_data_source[index + 2].type === 'on_condition') {

          right_table = await retrive_source_data(multi_select_data_source[index + 1].data, database_name_default)
          intermediate_data = await perform_join(left_table, right_table, multi_select_data_source[index].data, multi_select_data_source[index + 2].data)
          index = index + 2
          continue

        } else if (multi_select_data_source[index].data === 'INNER JOIN') {
          right_table = await retrive_source_data(multi_select_data_source[index + 1].data, database_name_default)
          intermediate_data = await perform_join(left_table, right_table, multi_select_data_source[index].data)
          index = index + 1
          continue
  ...
  const data_manupulaton_config = parse_data_filter_and_manupulation_part(m_select_literals)
  const manupulated_data = await do_data_modification(intermediate_data, data_manupulaton_config, true)
  const multi_column_info = extract_columns_info_of_multi_select(m_select_literals)
  const extracted_columns = select_given_columns_from_dataset(manupulated_data, multi_column_info)
