...
export function is_data_source_subquery(select_literals: string[]) {
  let found_from_index: number = null;
...
export async function parse_data_source(select_literals: string[]) {
  ...
  if (is_subquery) {
    ...
    if (splited_value.length > 2) {
      ...
      const database_name: string = null;
...
export async function parse_select_clause(select_statement: string) {
  ...
  const combined_sql_spaced_literals: string[] = combine_all_sql_spaced_literals(
    select_literals
  );
...
export async function single_select(
  ...
  const parsed_select = await parse_select_clause(select_string);
...
export async function do_data_modification(
  ...
  modification_arr = modification_arr.map((p) => {
    switch (p.type) {
      case 'WHERE':
        if (is_multi_query) {
          return {
