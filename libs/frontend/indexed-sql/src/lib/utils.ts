...
import { arithmatic_operator, comparision_operator, generalTrimOption } from './main.interface';
...
export const breakpoints_literals = ['ORDER BY', 'GROUP BY', 'WHERE', 'OFFSET', 'LIMIT'];
...
export function do_arithmatics(arithmatic_literals: any[]) {
    const do_maths = (left_operand: BigNumber, right_operand: BigNumber, type: arithmatic_operator) => {
        switch (type) {
            case '/':
    ...
    const manage_multiply_and_divide = (num: any[], cb: (result: BigNumber[]) => void) => {
    ...
    const manage_addition_and_substraction = (num: any[], cb: (result: BigNumber[]) => void) => {
        for (let index = 0; index < num.length; index++) {
            const operator = num[index];
            if (operator === '+' || operator === '-') {
                const left_operand = num[index - 1] as BigNumber;
...
export function findIndexDir<T>(arr: T[], search_element: T, direction: 'right' | 'left', start_index: number) {
    let found_index = -1;
    if (direction === 'right') {
...
export function found_number_of_comparision_operators(where_literals: string[]) {
    let number_of_comparision_operators_in_where_array = 0;
    where_literals.forEach((p) => {
        if (comparision_operators.includes(p)) {
            number_of_comparision_operators_in_where_array = number_of_comparision_operators_in_where_array + 1;
        }
    });
...
export function is_string_date(date_string: string) {
    ...
    if (array_date_data.length === 3) {
        ...
        if (array_time_data.length === 3) {
            ...
            const number_data_first_2 = array_date_data.slice(0, 2).filter((d) => !isNaN(Number(d)));
            ...
            const number_data_first_3 = array_date_data.filter((d) => !isNaN(Number(d)));
...
export function is_comparing_dates(left_operand: string, right_operand: string, operator: comparision_operator) {
...
export function compare_dates(left_operand: string, right_operand: string, operator: comparision_operator) {
...
export function is_comparing_string(left_operand: any, right_operand: any, operator: comparision_operator) {
...
export function compare_strings(left_operand: string, right_operand: string, operator: comparision_operator) {
    switch (operator) {
        case '=':
...
export function is_comparing_numbers(left_operand: any, right_operand: any, operator: comparision_operator) {
...
export function compare_numbers(left_operand: BigNumber, right_operand: BigNumber, operator: comparision_operator) {
    switch (operator) {
        case '=':
...
export function is_comparing_string_and_number(left_operand: any, right_operand: any, operator: comparision_operator) {
    const left_operand_type = typeof left_operand;
    const right_operand_type = typeof right_operand;
    if (left_operand_type === 'object' && right_operand_type === 'string') {
        return true;
    } else if (left_operand_type === 'string' && right_operand_type === 'object') {
        return true;
    } else {
        return false;
    }
...
export function compare_string_and_number(left_operand: any, right_operand: any, operator: comparision_operator) {
    switch (operator) {
        case '=':
            return false;
            break;
...
export function resolve_compare(left_operand: any, right_operand: any, operator: comparision_operator) {
    ...
    } else if (is_comparing_string_and_number(left_operand, right_operand, operator)) {
...
export function remove_char_form_end(start_char: string, end_char: string, arr: string[], cb: (result: string[]) => void) {
    const found_final_index = (start_c: string, end_c: string, str_arr: string[]) => {
        ...
        let found_final_indexx: number = null;
...
export async function union_maker(upper_data_i: any[], lower_data_i: any[], union_type: 'union_all' | 'union') {
    ...
    const upper_data = upper_data_i.map((val) => {
        if (Object.keys(val).length === Object.keys(reference_object).length) {
            ...
            // throw the error
            throw new Error('Object type and length should be equal when using union');
        }
    });
    ...
    if (upper_data.length !== 0 && lower_data.length !== 0) {
        ...
        if (union_type === 'union') {
            if (is_two_obejct_type_equal(upper_data[0], lower_data[0])) {
                const uniue_objects = await filter_unique_object_in_array([...upper_data, ...lower_data]);
...
export function group_by<T>(data: T[], key: string) {
    ...
    const group_now = (arr: any[], key_name: string, cb: (result: any[]) => void) => {
        // make copy of the first element
        const initial_copy = JSON.parse(JSON.stringify(arr[0]));
        // loop through the arr to find the match
        for (let index = 0; index < arr.length; index++) {
            const obj = arr[index];
            if (obj.hasOwnProperty(key_name) && initial_copy.hasOwnProperty(key_name)) {
...
export function sort_array_with_obj(arr: any[], key_name: string, type: 'ASC' | 'DESC') {
    if (arr.length === 0) {
        return;
    }
    ...
    if (type === 'ASC') {
        ...
        if (typeof first_obj[key_name] === 'number') {
            return arr.sort((a, b) => a[key_name] - b[key_name]);
        } else if (typeof first_obj[key_name] === 'string') {
            // check if the string is date
            if (is_string_date(first_obj[key_name])) {
                // string is date
                return arr.sort((a, b) => +new Date(a[key_name]) - +new Date(b[key_name]));
            } else {
                return arr.sort((a, b) => a[key_name].localeCompare(b[key_name]));
            }
        } else {
            return arr;
        }
        ...
        const first_obj = arr[0];
        if (typeof first_obj[key_name] === 'number') {
            return arr.sort((a, b) => b[key_name] - a[key_name]);
        } else if (typeof first_obj[key_name] === 'string') {
            // check if the string is date
            if (is_string_date(first_obj[key_name])) {
                // string is date
                return arr.sort((a, b) => +new Date(b[key_name]) - +new Date(a[key_name]));
            } else {
                return arr.sort((a, b) => b[key_name].localeCompare(a[key_name]));
            }
        } else {
            return arr;
        }
...
export function sort_similar_obj_in_array(arr: any[], similar_key: string, key_name: string, type: 'ASC' | 'DESC') {
    let initial_index = -1;
    ...
    for (let index = 0; index < arr.length; index++) {
        ...
        if (similar_ele.length === 0) {
            ...
            initial_index = index;
        } else if (similar_ele[similar_ele.length - 1][similar_key] === obj[similar_key]) {
            similar_ele.push(obj);
        } else {
    ...
    if (similar_ele.length !== 0 && initial_index > -1) {
        ...
        const sorted_items = sort_array_with_obj(similar_ele, key_name, type);
...
export function sort_by(data: any[], keys: { key_name: string; type: 'ASC' | 'DESC' }[]) {
    for (let index = 0; index < keys.length; index++) {
        ...
        if (index === 0) {
            ...
            data = sort_similar_obj_in_array(data, prev_key.key_name, key.key_name, key.type);
        }
    }
...
export async function unite_sources(sources_data: any[][], union_types: any[]) {
    const combine = async (sources: any[][], unions: any[], cb: (result: any[]) => void) => {
        if (sources.length - 1 === unions.length) {
            if (sources.length === 1) {
                ...
                const united_data = await union_maker(upper_data, lower_data, type.data.toLowerCase().split(' ').join('_') as any);
    ...
    return new Promise<any[]>((resolve, reject) => combine(sources_data, union_types, (result) => resolve(result)));
}

// parse order by

export function parse_order_by(order_by_literals: string[], is_multi_query = false) {
        ...
        .map((p, i) => {
            if (p.length === 1) {
                if (is_multi_query) {
                    ...
                    if (!sort_by_types.includes(column_name.toUpperCase()) && typeof column_name === 'string') {
                    ...
                    const column_name = column_name_splited[column_name_splited.length - 1];
                    if (!sort_by_types.includes(column_name.toUpperCase()) && typeof column_name === 'string') {
                        return { key_name: column_name, type: 'ASC' };
                    } else {
                        return null;
                ...
                if (is_multi_query) {
                    ...
                    if (sort_by_types.includes(sort_type) && typeof column_name === 'string') {
...
export function table_schema_unique_string(database_name: string, table_name: string) {
...
export async function retrive_next_database_version(database_name: string) {
    try {
        ...
        const tnx = database_connection.transaction(indexed_sql_config_versions, 'readonly');
        const current_version = await tnx.objectStore(indexed_sql_config_versions).get(database_name);
...
export async function upgrade_database_version(database_name: string, version: number) {
    const database_connection = await openDB(indexed_sql_config_database, 1);
    const tnx = database_connection.transaction(indexed_sql_config_versions, 'readwrite');
    const inserted_id = await tnx.objectStore(indexed_sql_config_versions).put(version, database_name);
    await tnx.is_complete();
    database_connection.close();
    return 'OK';
}
...
export async function setup_initial_config_tables() {
    // setup the
    const connection_db = await openDB(indexed_sql_config_database, 1, (upgradeDB) => {
        if (!upgradeDB.objectStoreNames.contains(indexed_sql_config_schema_name)) {
...
export const convert_sql_data_to_normal_data = (data: any[]) => {
    return data.map((p) => convert_sql_object_to_normal_object(p));
};
