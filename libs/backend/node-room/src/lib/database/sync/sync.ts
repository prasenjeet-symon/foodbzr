...
export class OfflineSync {
  ...
  constructor(
    ...
    this.childDatabaseParamObject = this.childDatabaseParamObject
      ? this.childDatabaseParamObject
      : {};
    if (childDatabaseName) {
      // there is child database
      const childDB = this.rootDatabaseInstance.getChildDatabase(
        childDatabaseName
      );
      if (childDB) {
        const childDBInstance = new childDB(
          ...Object.values(this.childDatabaseParamObject)
        );
    ...
    this.socket.on('sync_table', (data) => {
      ...
      this.perform_database_operation(
        main_table_name,
        unsynced_data,
        synced_data_uuid,
        lastDaoIndex
  ...
  private perform_database_operation = async (
    main_table_name: string,
    unsynced_data: any[],
    synced_data_uuid: string[],
    lastDaoIndex: number
  ) => {
    const fetch_condition = this.clientDatabase.getCacheFetchCondition(
      main_table_name
    ); // this fetch condition can be 'undefined' or ''
    const log_table = `${main_table_name}_UD_LOG`; // Name of the log table
    synced_data_uuid = synced_data_uuid.map((p) =>
      parse_sql_string(p)
    ) as string[];
    ...
    const SQL_extract_new_data_from_server = `
            ...
            WHERE
            row_uuid NOT IN ( ${
              synced_data_uuid.length === 0 ? `''` : synced_data_uuid.join(', ')
            } ) 
    ...
    const connection: any = DatabaseConnection.getInstance().create_mysql_connection(
      this.mysql_config,
      true,
      this.database_name
    );
    const start_transaction = await this.do_sequencial_single_query(
      connection,
      `START TRANSACTION;`
    );

    const extracted_new_data = await this.do_sequencial_single_query(
      connection,
      SQL_extract_new_data_from_server
    );
    if (unsynced_data.length !== 0) {
      await this.do_sequencial_single_query(
        connection,
        SQL_insert_new_data_to_server
      );
    }

    let read_daos = (await this.do_sequencial_single_query(
      connection,
      SQL_read_daos_after_local_index
    )) as any[];
  ...
  private do_sequencial_single_query = (
    connection: any,
    single_query_statement: string
  ) => {
