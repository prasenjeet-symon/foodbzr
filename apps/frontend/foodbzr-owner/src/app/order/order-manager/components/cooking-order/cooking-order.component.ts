...
export class CookingOrderComponent implements OnInit, OnDestroy {
    ...
    public cooking_arr: IGetOrderStatusGroupedKitchen[] = [];
    ...
    public initScreen(can_show_loading = true) {
        this.platform.ready().then(() => {
            ...
            this.fetch_order_status_pending__.observe(this.daosLife).subscribe((val) => {
                ...
                this.ngZone.run(() => {
                    this.cooking_arr = group_order_status_in_kitchen(val.sort((a, b) => +new Date(b.food_order_date_created) - +new Date(a.food_order_date_created)));
                });
            });
    ...
    cookingOrderTrackerGroup(index: number, value: IGetOrderStatusGroupedKitchen) {
