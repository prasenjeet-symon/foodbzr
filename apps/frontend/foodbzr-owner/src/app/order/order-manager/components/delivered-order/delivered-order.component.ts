...
export class DeliveredOrderComponent implements OnInit, OnDestroy {
    ...
    public delivered_orders: IGetOrderStatusGroupedKitchen[] = [];
    ...
    public initScreen(can_show_loading = true) {
        this.platform.ready().then(() => {
            ...
            this.fetch_order_status_pending__.observe(this.daosLife).subscribe((val) => {
                ...
                this.ngZone.run(() => {
                    this.delivered_orders = group_order_status_in_kitchen(val.sort((a, b) => +new Date(b.food_order_date_created) - +new Date(a.food_order_date_created)));
                });
            });
