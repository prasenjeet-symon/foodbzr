...
export class DeliveredOrderComponent implements OnInit, OnDestroy {
    ...
    public delivered_orders: IGetOrderStatus[] = [];
    ...
    public initScreen(can_show_loading = true) {
        this.platform.ready().then(() => {
            ...
            this.fetch_order_status_pending__.observe(this.daosLife).subscribe((val) => {
                ...
                this.ngZone.run(() => {
                    this.delivered_orders = val.sort((a, b) => +new Date(b.food_order_date_created) - +new Date(a.food_order_date_created));
                });
            });

            if (can_show_loading) {
                this.loading.showLoadingScreen().then(() => {
                    this.fetch_order_status_pending__.fetch('delivered', [this.partner_row_uuid]).obsData();
                });
            } else {
                this.fetch_order_status_pending__.fetch('delivered', [this.partner_row_uuid]).obsData();
            }
