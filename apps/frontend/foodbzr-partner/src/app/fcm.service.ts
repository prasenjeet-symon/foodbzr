...
export class FcmService {
    ...
    constructor(private router: Router, private toast: ToastController, private platform: Platform) {
        this.entity_row_uuid = localStorage.getItem('partner_row_uuid');
    }
    ...
    private saveToDB(key: string) {
        this.platform.ready().then(async () => {
            ...
            (await insert_push_message__.fetch('partner', this.entity_row_uuid, key)).obsData();
            daoLife.softKill();
        });
    }

    /** handle routing */
    public handleRouting(notification: PushNotification) {
